...
public class AdminController {
	...
	//GET 방식으로 업데이트 폼만 보여줍니다.
		MemberVO memberVO = memberService.readMember(user_id);
		model.addAttribute("memberVO", memberVO);
		return "admin/member/membr_update";
	}
	...
	public String member_update(PageVO pageVO, MemberVO memberVO) throws Exception {
		...
		//redirect를 사용하는 목적은 새로고침 했을때, 위updateMember메서드를 재 실행방지 목적입니다.
	...
	public String member_delete(RedirectAttributes rdat, @RequestParam("user_id") String user_id) throws Exception{
		...
		//redirect페이지 이동시 전송값을 숨겨서 보내는 역할 클래스 RedirectAttributes 입니다.
	...
	public String member_list(@ModelAttribute("pageVO")PageVO pageVO, Model model)throws Exception {
		...
		/*@RequestParam(value="search_type",required=false) String search_type, @RequestParam(value="search_keyword",required=false) String search_keyword, Model model) 
		 * 옛날 검색 코드
		 * String[][] members = {
		...
		//검색된 전체 회원 며수 구하기 서비스 호출
		int countMember = 0;
		countMember = memberService.countMember(pageVO);
		pageVO.setTotalCount(countMember);//전체 회원의 수를 구한 매개변수로 입력
