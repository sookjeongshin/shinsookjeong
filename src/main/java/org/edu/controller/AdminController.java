...
public class AdminController {
	...
	@Inject
	IF_BoardDAO boardDAO; // Controller-Service-DAO-Mapper-DB

	@Inject
	IF_MemberService memberService;// 멤버인터페이스를 주입받아서 memberService오브젝트 변수를 생성.
	// get방식은 url전송방식(아무데서나 브라우저주소에 적으면 실행됨), post는 폼전송방식(해당페이지에서만 작동가능)

	@RequestMapping(value = "/admin/board/board_delete", method = RequestMethod.POST)
	public String board_delete(RedirectAttributes rdat, PageVO pageVO, @RequestParam("bno") Integer bno)
			throws Exception {
		// 기족등록된 첨부파일 폴더에서 삭제할 UUID파일명 구하기 아래
	...
	public String board_update(@RequestParam("bno") Integer bno, @ModelAttribute("pageVO") PageVO pageVO, Model model)
			throws Exception {
		...
		for (AttachVO file_name : files) {// 세로데이터를 가로데이터로 변경하는 로직
			save_file_names[cnt] = file_name.getReal_file_name();
			real_file_names[cnt] = file_name.getSave_file_name();
			cnt = cnt + 1;
		}
		/*
		 * for (HashMap<String, Object> file_name : files_noUse) {// 세로데이터를 가로데이터로 변경하는
		 * 로직 save_file_names[cnt] = (String) file_name.get("save_file_name");// 형변환
		 * cast real_file_names[cnt] = (String) file_name.get("real_file_name"); cnt =
		 * cnt + 1; }
		 */
	...
	public String board_update(RedirectAttributes rdat, @RequestParam("file") MultipartFile[] files, BoardVO boardVO,
			PageVO pageVO) throws Exception {
		// 기존 등록된 첨부파일 목록 구하기
		List<AttachVO> delFiles = boardService.readAttach(boardVO.getBno());
		//List<HashMap<String, Object>> delFiles = boardService.readAttach_noUse(boardVO.getBno());
			...
			if (file.getOriginalFilename() != "") {// 첨부파일명이 있으면
				...
				for (AttachVO file_name : delFiles) {
					if (index == sun) {// index는 첨부파일개수 , sun삭제할 개별순서
						File target = new File(commonController.getUploadPath(),
								file_name.getSave_file_name());
						if (target.exists()) {
				...
				// 신규파일 폴더에 업로드 처리
				save_file_names[index] = commonController.fileupload(file);// 폴더에 업로드 저장완료
	...
	public String board_write(RedirectAttributes rdat, @RequestParam("file") MultipartFile[] files, BoardVO boardVO)
			throws Exception {
		...
		String[] real_file_names = new String[files.length];
		int index = 0;
		// 첨부파일이 있으면, 첨부파일 업로드처리 후 게시판 DB저장 + 첨부파일 DB저장
	...
	public String board_view(@ModelAttribute("pageVO") PageVO pageVO, @RequestParam("bno") Integer bno, Model model)
			throws Exception {
		// jsp로 보낼 더미 데이터 boardVO에 담아서 보낸다.
		// 실제로는 아래처럼 더미데이터를 만드것이 아닌
		// 쿼리스트링(질의문자열)로 받아온 bno(게시물 고유번호)를 이용해서 DB에서
		// select * from tbl_boarad where bno = ? 마이바티스 실행이 된 결과값이 BoardVO형으로 받아서
		// jsp보내줌.
		// '3', '새로운 글을 넣습니다. ', '새로운 글을 넣습니다. ', 'user00', '2019-10-10 12:25:36',
		// '2019-10-10 12:25:36', '0', '0'
		/*
		...
		// ]
		List<AttachVO> files = boardService.readAttach(bno);
	//	List<HashMap<String, Object>> files = boardService.readAttach_noUse(bno);
		String[] save_file_names = new String[files.size()];
		String[] real_file_names = new String[files.size()];
		int cnt = 0;
		...
		/*
		 * for (HashMap<String, Object> file_name : files) { save_file_names[cnt] =
		 * (String) file_name.get("save_file_name"); real_file_names[cnt] = (String)
		 * file_name.get("real_file_name"); cnt = cnt + 1; }
		 */
		// 배열형출력값(가로) {'save_file_name0','save_file_name1',...}
		boardVO.setSave_file_names(save_file_names);
		boardVO.setReal_file_names(real_file_names);
	...
	public String board_list(@ModelAttribute("pageVO") PageVO pageVO, Model model) throws Exception {
		...
		// model.addAttribute("pageVO", pageVO); @애노테이션으로 대체
	...
	public String member_update(@RequestParam("user_id") String user_id, @ModelAttribute("pageVO") PageVO pageVO,
	...
	public String member_view(@ModelAttribute("pageVO") PageVO pageVO, @RequestParam("user_id") String user_id,
			Model model) throws Exception {
