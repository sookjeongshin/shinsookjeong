...
public class LoginController {
	...
	public String login_seccess(HttpServletRequest request, RedirectAttributes rdat) throws Exception{
		...
		HttpSession session = request.getSession();//진입 전 단계에서 발생한 세션(로그인 절차로 생성된 세션)을 가져오는 절차
		if(enabled) {//인증처리가 true라면 아래에서 세션 등록 시작
			//자바8이상에서만 지원되는 람다식 사용해서 DB쿼리에서 GET값 getAuthority()비교구문 처리 levels 변수에 권한 부여.
	...
	public String login() throws Exception{
		//BCrypt암호화 match 메서드으로 확인
				...
				//System.out.println(bcryptPassword);//테스트시 이 암호를  DB회원테이블에 입력하면
