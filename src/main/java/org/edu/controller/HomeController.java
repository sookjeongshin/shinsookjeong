 ...
 * http://localhost:8080/ 요청을 받았을때, 변수를 만들어서 크롬화면 또는 화면에 뿌려준다.
 */
@Controller
public class HomeController {
	...
	public String contact_send() {
		//데이터 전송후에 다른 페이지이동이 필요합니다.새로고침=자동등록을 방지하기 위해서.
		return "redirect:/blog";
	}
	@RequestMapping(value="/contact",method=RequestMethod.GET) //겟방식은 폼페이지를 보여주는 역
...
//상단 리퀘스트매핑에서 지정한 경로로 접근을 하게되면, 아래에 있는 index()메서드를 실행하라.
	...
	public String home(Locale locale, Model model) {
		...
		System.out.println("위에 Locale클래스를 사용하는 이유는 다국어 지원");
		System.out.print("함수-C언어나자바스크립트-와 매서드-자바, 스프링에서 사용함-는 같은 대상을 가리킵니다.");
		System.out.print("함수-메서드는 함수명('매개변수-입력값'){구현내용}형식이고, 입력값->출력값 구현됩니다.");
		Date date = new Date();//Data 날짜관련 클래스형 변수 date 선언했습니다. date변수 메서드를 사용 가능하게 됨 
		//data 변수는 실행가능한 변수=클래스형변수=Object오브젝트=인스턴스 라고 합니다.
		DateFormat dateFormat = DateFormat.getDateTimeInstance(DateFormat.LONG, DateFormat.LONG, locale);
		//DataFormat 클래스형 변수 dataFormat 이 선언되었고, 실행가능한 변수로 되었습니다. (위) 
		...
		//스프링에서 뷰단데이터 이동(전송)에 대해서 살펴 보았습니다.
	}*/
