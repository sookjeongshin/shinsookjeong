...
/**
 * 앱을 위한 홈페이지 요청(request)을 처리한다(아래). http://localhost:8080/ 요청을 받았을때, 변수를 만들어서
 * 크롬화면 또는 콘솔에 뿌려준다.
 */
@Controller
public class HomeController {
	...
	@Inject SecurityCode securityCode;
	
	@Inject
	private CommonController commonController;
	//전역 홈페이지에서  스프링 진입전 발생한느 에러 페이지 처리
	...
	public String board_view(@RequestParam("bno") Integer bno, @ModelAttribute("pageVO")PageVO pageVO, Model model) throws Exception{
		...
		//model.addAttribute("pageVO", pageVO); //@ModelAttribute로 대체해서 jsp로 오브젝트를 전송
		//첨부파일 데이터 jsp 뷰단으로 보내기 (아래)
		List<AttachVO> files = boardService.readAttach(bno);
		//[
		//{'save_file_name':저장된 파일명, 'real_file_name':DB에저장된파일명},
		//{'save_file_name':저장된 파일명, 'real_file_name':DB에저장된파일명},
		//]
		...
		model.addAttribute("ckeckImgArray", commonController.getCheckImgArray());
		return "home/board/board_view";
	}
	//사용자 홈페이지 게시판 쓰기 매핑(POST) 오버로드 (매개변수의 개수또는 타입이 틀린) 메서드이용
	// jsp에서board_write메서드를 호출합니다->호출할때 폼의 필드값을 컨트롤러로 보냅니다.
	// 컨트롤러에서 받을때 사용하는 매개변수 BoardVO boardVO입니다.
	// 위에서 받으 boardVO 를 DAO에서 받아서 DB테이블에 쿼리로 입력합니다.
	...
	public String board_list(@ModelAttribute("pageVO") PageVO pageVO, Model model) throws Exception {
		...
		pageVO.setQueryPerPageNum(10); //1페이지당 보여줄 개수
	 ...
	 * //data 변수는 실행가능한 변수=클래스형변수=Object오브젝트=인스턴스 라고 합니다.(위) DateFormat dateFormat =
	 * DateFormat.getDateTimeInstance(DateFormat.LONG, DateFormat.LONG, locale);
	 * //DateFormat 클래스형 변수 dateFormat 이 선언되었고, 실행가능한 변수로 되었습니다.(위) String
	 * formattedDate = dateFormat.format(date);//변수가 실행됩니다. //위 변수가 실행되어서 출력된 결과값이
	 * String클래스형 변수 formattedDate변수에 담기게 됩니다.
	 * model.addAttribute("TomcatserverTime", formattedDate );//serverTime 변수값으로
	 * jsp파일로 이동. //위 model이라는 클래스형 변수를 이용해서, serverTime변수값을 아래 home(생략 .jsp)로 전송해
	 * 줌. System.out.println("현재 서버의 시간은 " + formattedDate + " 입니다."); return
	 * "home";//결과적으로 return 출력값이 home(생락 .jsp)에 연동됩니다. //스프링에서 뷰단으로 데이터 이동(전송)에 대해서
	 * 살펴 보았습니다. }
	 */
