...
public class JsonDataController {
	...
	//RestAPI서버: 오픈차트js에 투표한 값을 저장(인서트 |업데이트 합니다.)
	...
	public ResponseEntity<String> androidDelete(@PathVariable("user_id") String user_id) {
		...
		try {
			...
			entity = new ResponseEntity<>("success", HttpStatus.OK);//200값을 success텍스트로 전송
		} catch (Exception e) {
			entity = new ResponseEntity<>(HttpStatus.BAD_REQUEST);
		}
		return entity;
...
//RestAPI 서버 회원목록을 출력 기능(아래)
@RequestMapping(value="/android/list", method=RequestMethod.POST)
public ResponseEntity<List<MemberVO>> androideMember() {
