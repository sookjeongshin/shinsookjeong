...
public class DebugAdvice {
	 ...
	 * ProceedingJoinPoint.proceed()메서드는 특이하게도 Exception보다 상위로 예외 전달합니다.
	 * 즉, 일반 내부 Exception에러처리보다 외부에서 ProceedingJoinPoint.proceed()메서드로 먼저처리 됩니다.
	 * proceed()메서드 생성에 Throwable을 사용하고, 시간을 체크하는 기능을 작성 할 수 있습니다.
	 * 위 시간 체크하는 기능이 필요한 이유는? 개발자가 만든 다양한 메서드의 시작과 끝 시간을 체크가 가능합니다.
	 * 예를 들면, 개발자가 만든 클래스를 실행시켰을때, 5~10분 이상 느리게 진행되는 현상이 생깁니다.
