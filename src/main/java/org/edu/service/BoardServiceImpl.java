...
public class BoardServiceImpl implements IF_BoardService {
	...
	@Override // 아래 메서드는 구현이 2개있는데, 조회수 업데이트 정상처리 후 조회 에러발생시 업데이트한 조회수를 원상복귀 시키는 작업이 트랜잭션처리
	...
	public void insertBoard(BoardVO boardVO) throws Exception {
		...
		if (save_file_names == null) {
			return;
		} // 배열첨부파일이 없으면 진행 빠져나감
		for (String save_file_name : save_file_names) {// 첨부파일이 1개일때는 1번만 반복됩니다.
			if (save_file_name != null) { // 첨부파일배열에서 배열값이 있는 경우만
