	...
	<div id="container">
		...
		<div class="bodytext_area box_inner">
			...
			<form id="search_form" name="search_form" action="/home/board/board_list" class="minisrch_form">
				<fieldset>
					<legend>검색</legend>
					<input name="serch_type" value="all" type="hidden">
			...
			<table class="bbsListTbl" summary="번호,제목,조회수,작성일 등을 제공하는 표">
				...
				<c:forEach items = "${board_list}" var="boardVO" varStatus="status">
						<tr>
						<td><!-- 전체게시물-(현재페이지x1페이지당보여줄개수)+1페이지당보여줄개수-현재인덱스값 -->
                      ${pageVO.totalCount-(pageVO.page*pageVO.queryPerPageNum)+pageVO.queryPerPageNum-status.index}
                      </td>
						<td class="tit_notice">
						<a href="/home/board/board_view?bno=${boardVO.bno}&spage=${pageVO.page}&search_type=${pageVO.search_type}">
			...
			<c:forEach begin="${pageVO.startPage}" end="${pageVO.endPage}" var="idx">
				...
				<span class='pagenum <c:out value="${(pageVP.page==idx)?'currentpage':''}" />'>${idx}</span></a>
			</c:forEach>
