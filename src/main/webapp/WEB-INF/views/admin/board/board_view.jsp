						...
						<div class="card-body">
							...
							<c:forEach var="index" begin="0" end="1">
								<c:if test="${boardVO.save_file_names[0] !=null}">
									...
									<p class="text-muted">
										<a
											href="/download?save_file_name=${boardV0.save_file_names[index]}&real_file_name=${boardVO.real_file_name[index]}">
											${boardVO.real_file_names[index]}-파일다운로드${index }- </a>
										<c:set var="fileNameArray"
											value="${fn:split(boardVO.save_file_names[index],'.')}" />
										<c:set var="extName"
											value="${fileNameArray[fn:length(fileNameArray)-1]}" />
										...
										<c:choose>
											<c:when test="${fn:containsIgnoreCase(chkImgArray,extName)}">
												<!-- true이면 이미지 파일이란 의미 -->
												<img style="width: 100%;"
													src="/image_preview?save_file_name=${boardVO.save_file_names[index]}&real_file_name=${boardVO.real_file_names[index]}">
											</c:when>
											<c:otherwise>
...
<script>
$(document).ready(function(){
	$("updateReflyBtn").on("click",function(){
		var rno = $("#rno").val();//모달창의 input태그값  변수지정
		var reply_text=""; = $("reply_text_modal").val(); //모달창의 input태그값 변수지정
		$.ajax({
			type:"patch",
			...
			dateType:"text", //반환값RestAPI에서 반환되는 값
			success:function(result){
