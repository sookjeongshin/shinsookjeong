...
/**
 * JUnit 자바단위테스트이고, DataSource 지정후 DB(Hsql,Mysql,Oracle) 접속, 데이터베이스 테이블에 Create,
 * Read, Update, Delete 테스트용 클래스 제이유닛4클래스를 사용. 단위테스트는 톰캣이 실행되지 않아도 작동이 되야 합니다.
...
public class DataSourceTest {
	...
	@Inject // 사용하면 안되는 이유: 클래스상단에 @Controller, @Service, @Repository, @Component 이런내용만
			// @Inject합니다.
	MemberVO memberVO;// 기존자바처럼 new MemberVO() 오브젝트를 생성하지않고, 주입해서사용.

	// 기존자바처럼 new MemberVO() 오브젝트를 생성하지않고, 주입해서사용.
	...
	public void updateMember() throws Exception {
		// CRUD 중 Update 테스트 구현 특징, user_id는 프라이머리 키 이기 때문에 수정대상이 아닙니다.
		...
		String user_id = memberVO.getUser_id();// memberVO의 오브젝트의 데이터는 1개의 레코드이기 때문에 반환값이 1개
	...
	public void deleteMember() throws Exception {
		// CRUD 중 Delete 테스트 구현 쿼리 >DAO >Service >memberDAO주입받은 오브젝트사용
		memberDAO.deleteMember("user_20201215145633381");
	}
