...
public class OpenApi {
	...
	public static void main(String[] args) {
		//메인스레드는 1개 다른스레드를 추가로 실행할떄, Runnable메서드를 사용합니다.(아래)
		//추가스레드를 스케줄로 실행할떄 실행간격 변수(5초)
		int sleepSec = 5;
		//주기적인 스레드작업(Concurrent동시작업)을위한 코딩:new 키워드로 실행가능한 오브젝트 변수 exec변수 생성.
		...
		//staticTest.test(); //노스태틱은 오브젝트생성후 접근이 가능
		//StaticTest.test(); //스태틱에서는 오브젝트생성없이 클래스로 바로 접근이 가능 메서드, 변수도 가능.
		/*
		int sum = 0;
		while(sum < 10) {
